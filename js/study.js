...
// Filter/Find array
let array = [
    { GOTY: `Sekiro`, year: '2019'},
    { GOTY: `The last of us part 2`, year: '2020'},
    { GOTY: `It takes two`, year: '2021'},
    { GOTY: `Elden ring`, year: '2022'},
    { GOTY: `Baldur's Gate 3`, year: '2023'},
    { GOTY: `Astro bot`, year: '2024'}
];
let filter = array.filter( (item) => {
    // return item.length > 3; // Trả về những item có độ dài > 3
    return item.year <= '2022'; // Trả về những item year <= 2022 
});
let find = array.find( (item) => {
    return item.year <= '2022'; 
    // Không giống filter, find sẽ trả về item thõa mãn điều kiện đầu tiên
    // Lần lượt Sekiro , TLOU2, ITT, ER thỏa điều kiện 
    // nhưng find sẽ trả về kq thỏa mãn đầu tiên là Sekiro
});
console.log(filter, find);

// Map
let mapArray = [ 'HTML', 'CSS', 'JS', 'React', 'Python' ];
let map = mapArray.map( (item) => {
    item = `Language ` + item;
    return item;
}); // Giống for lặp từng phần tử trong array nhưng map thì tạo một array mới
console.log(map); // Thường dùng nhiều trong react
// for (let i = 0; i < mapArray.length; i++) {
//     mapArray[i] = `Language ` + mapArray[i];
// }
